...
def BaseandArmSpeeds(end_effector_twist, thetalist):
    ...
    # print(f'J: {Jrob}')
    wheel_and_joints = np.matmul(np.linalg.pinv(Jrob, 1e-3), end_effector_twist)
    # print(f'wheel and joint speeds: {wheel_and_joints}')
    return wheel_and_joints


def FeedbackControl(X, Xd, Xd_next, Kp, Ki, dt, tot_Xerr):
    """
    ...
    V = (np.matmul(adX_Xd, Vd)) + (np.dot(Kp, Xerr)) + (np.dot(Ki, tot_Xerr))
    # print(f'Vd: {Vd}')        
